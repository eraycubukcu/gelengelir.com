...
export const Profile: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center py-16 px-2 animate-fade-in-up">
      {/* Profil Kartı */}
        ...
        <div className="mt-20 w-full flex flex-col items-center">
          ...
          <div className="flex gap-6 mb-4">
            <div className="flex flex-col items-center">
              <span className="text-xl font-bold text-violet-600 dark:text-violet-400">{joinedCount}</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Katıldığı Etkinlik</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-xl font-bold text-amber-500 dark:text-amber-400">{myAdsCount}</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Açtığı İlan</span>
            </div>
          </div>
          {/* Rozetler */}
          <div className="flex gap-3 mb-4">
            {badges.map((b, i) => (
              <div key={i} className="flex flex-col items-center bg-indigo-50 dark:bg-gray-700/50 px-3 py-2 rounded-xl shadow text-xs">
                <span className="text-2xl mb-1">{b.icon}</span>
                <span className="font-semibold text-violet-600 dark:text-violet-400">{b.label}</span>
                <span className="text-gray-500 dark:text-gray-400">{b.desc}</span>
              </div>
            ))}
          </div>
      ...
      {/* Etkinlikler Sekmesi */}
      <div className="w-full max-w-5xl bg-white dark:bg-gray-800/50 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-8 mt-2 animate-fade-in-up">
        <div className="flex gap-4 mb-8 justify-center">
          <button
            className={`px-6 py-2 rounded-full font-semibold transition-all text-lg ${tab === 'upcoming' ? 'bg-gradient-to-r from-violet-500 to-amber-400 text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
            onClick={() => setTab('upcoming')}
          >
            Beklenen Etkinlikler
          </button>
          <button
            className={`px-6 py-2 rounded-full font-semibold transition-all text-lg ${tab === 'past' ? 'bg-gradient-to-r from-violet-500 to-amber-400 text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
            onClick={() => setTab('past')}
          >
            Geçmiş Etkinlikler
          </button>
          <button
            className={`px-6 py-2 rounded-full font-semibold transition-all text-lg ${tab === 'myads' ? 'bg-gradient-to-r from-violet-500 to-amber-400 text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
        ...
        {tab === 'upcoming' ? (
          upcomingEvents.length > 0 ? (
            ...
            <div className="flex flex-col items-center text-gray-400 dark:text-gray-500 py-12 animate-pulse">
              <span className="text-6xl mb-4">🎈</span>
              <p className="text-lg text-gray-500 dark:text-gray-400">Henüz beklenen etkinliğiniz yok.</p>
            </div>
          )
        ) : tab === 'past' ? (
          pastEvents.length > 0 ? (
            ...
            <div className="flex flex-col items-center text-gray-400 dark:text-gray-500 py-12 animate-pulse">
              <span className="text-6xl mb-4">⏳</span>
              <p className="text-lg text-gray-500 dark:text-gray-400">Geçmiş etkinliğiniz yok.</p>
            </div>
          )
        ) : (
          myAds.length > 0 ? (
            ...
            <div className="flex flex-col items-center text-gray-400 dark:text-gray-500 py-12 animate-pulse">
              <span className="text-6xl mb-4">📢</span>
              <p className="text-lg text-gray-500 dark:text-gray-400">Henüz bir ilan oluşturmadınız.</p>
            </div>
          )
        )}
      </div>
